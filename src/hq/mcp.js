...
export function buildMcpServer({ core, notices, registry, version }) {
  const server = new McpServer({ name: 'storyfront', version });
  ...
  const tool = (name, description, shape, fn) => {
    server.registerTool(name, { description, inputSchema: shape }, async (args) => {
      try {
        ...
        if (err instanceof SfError) return { isError: true, content: [{ type: 'text', text: err.message }] };
        throw err;
      }
    });
  };
  ...
  // --- everyone ---
  tool('sf_status', 'Team, workday and ticket overview. Includes "control" if the workday is ending or stopped.', { as }, ({ as }) => core.status(as));
  tool('sf_ticket', 'Full details of one ticket: body, refs, checkpoint, work log, reviews, commits, thread, and its git branch (for reviews).', { as, id }, ({ as, id }) => core.ticketInfo(as, id));
  tool('sf_next', 'Ready tickets for you: ones assigned to you, then unassigned ones matching your domain.', { as }, ({ as }) => core.next(as));
  tool(
    'sf_claim',
  ...
  tool('sf_log', 'Add a progress note to a ticket\'s work log.', { as, id, text: z.string() }, ({ as, id, text }) => core.log(as, id, text));
  tool(
    'sf_block',
  ...
  tool('sf_checkpoint', 'Save a checkpoint without parking (e.g. before End day).', { as, id, done: z.string(), next: z.string(), files: list('Files touched') }, ({ as, id, ...f }) => core.checkpoint(as, id, f));
  tool(
    'sf_submit',
  ...
  tool('sf_review', 'Review someone else\'s ticket that is in review.', { as, id, verdict: z.enum(['approve', 'changes']), notes: z.string().optional() }, ({ as, id, ...f }) => core.review(as, id, f));
  tool('sf_attach', 'Attach a screenshot (png/jpg/webp) to a ticket.', { as, id, path: z.string().describe('Absolute path or path from the project root'), caption: z.string().optional() }, ({ as, id, ...f }) => core.attach(as, id, f));
  tool('sf_handoff', 'Leave a handoff note before you stop: done / in progress / next / waiting on.', { as, done: z.string(), inProgress: z.string().optional(), next: z.string(), waitingOn: z.string().optional() }, ({ as, ...f }) => core.handoff(as, f));
  tool(
    'sf_post',
  ...
  tool('sf_read', 'Your unread messages: DMs and mentions.', { as }, ({ as }) => core.read(as));
  tool(
    'sf_ask',
    'Ask a teammate (or "lead") a question. Returns a question ID. Don\'t wait for the answer: park the ticket with sf_block if you are stuck.',
    { as, to: z.string(), question: z.string(), ticket: z.string().optional(), links: list('Links to what you are asking about') },
    ({ as, ...f }) => core.ask(as, f),
  );
  tool('sf_answer', 'Answer a question (Q-n) addressed to you.', { as, q: z.string(), answer: z.string() }, ({ as, q, answer }) => core.answer(as, q, answer));
  tool(
    'sf_decide',
  ...
  tool('sf_facts', 'Answers the human has already given. Check before asking anything.', { as }, () => core.facts());
  tool(
    'sf_lookup',
    ...
    ({ ref }) => {
      ...
      if (!hit) throw new SfError(`Nothing is registered as ${ref}.`);
  ...
  tool('sf_design_approve', 'Approve a design doc you depend on.', { as, path: z.string() }, ({ as, path }) => core.designApprove(as, path));
  tool('sf_memory', 'Read your identity and your two memories (work, comms).', { as }, ({ as }) => core.memoryRead(as));
  tool(
    'sf_memory_write',
  ...
  tool('sf_memory_archive', 'Move older detail out of memory into your searchable archive.', { as, text: z.string() }, ({ as, text }) => core.memoryArchive(as, text));
  tool('sf_env', 'Your own ports and database name, so you never collide with teammates.', { as }, ({ as }) => core.env(as));

  // --- lead only ---
  tool('sf_start_day', 'Lead: start the workday and get a briefing.', { as }, ({ as }) => core.startDay(as));
  tool('sf_end_day', 'Lead: close the workday after everyone has handed off.', { as }, ({ as }) => core.endDay(as));
  tool(
    'sf_wait',
    ...
    async ({ as, seconds }) => {
      if (as !== cfg.leadId) throw new SfError('Only the lead waits for notices.');
  ...
  tool('sf_assign', 'Lead: assign a ticket to an agent.', { as, id, agent: z.string() }, ({ as, id, agent }) => core.assign(as, id, agent));
  tool('sf_reopen', 'Lead: reopen a ticket (e.g. after a failed merge or a change request).', { as, id, reason: z.string() }, ({ as, id, reason }) => core.reopen(as, id, reason));
  tool(
    'sf_escalate',
