---
name: mc-ui-sprint
description: Run the UI sprint for a release or epic with the human, the one session they sit in. Build clickable screens with fake data, iterate on their feedback, then lock the screens and derive the API contract from them.
---
...
1. **Scope.** List the release's (or epic's) screens and the navigation between them in one short message, e.g. `Sign-up → Tasks (empty) → New task → Tasks (filled)`. Ask the human to confirm or adjust. This is the only planning question.
...
4. **Sign-off.** When the human approves:
   - log it: `mc_decide` titled `UI signed off: <release or epic>`, with links to the screenshots
   ...
   - update the tickets: UI tickets now wire the real contract; backend tickets implement it. In UI-MVP mode that wiring is the next release (R2): plan it with `/mc-plan-release`
5. Hand over to the team with `/mc-start`. From here the human is only asked about decisions that are theirs.

For a release or epic **without UI** (MVP mode), don't run this. Build the thinnest end-to-end slice first, show it through an API explorer (Swagger UI) or a CLI run, and revise from the human's feedback.
