...
2. Call `mc_start_day` with `as: "lead"`. Read the briefing: team, handoffs from yesterday, ready / blocked / in-review tickets, open questions, messages from the human.
...
**d. Wait.** Call `mc_wait` with `as: "lead"`. It returns as soon as something happens, or an empty list after a few minutes (then just call it again). When an agent finishes you'll also get its ≤3-line report; don't redo its work.
