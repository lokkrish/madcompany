...
6. Tell the human: restart Claude Code once so the new agents load, then `/mc-plan-epic`. They can change any agent's model, hire or remove agents later in HQ → Team, and change the lead's model with `/model`. To bring teammates or a client into HQ, see `npx madcompany people add` and `npx madcompany hq --share`.
