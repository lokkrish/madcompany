...
test('normal development commands are allowed', () => {
  const allowed = [
    ...
    'psql postgres://storyfront:storyfront@localhost:5433/sf_arjun -c "select 1"',
  ];
  for (const cmd of allowed) assert.equal(checkCommand(cmd, { root }), null, `should allow: ${cmd}`);
  assert.equal(checkCommand('git push origin sf/app-1', { root, allowPush: true }), null);
});
...
test('Stop now blocks everything except Storyfront tools', () => {
  ...
  assert.equal(decide({ tool_name: 'mcp__storyfront__sf_handoff', tool_input: {} }, { root: r }), null);
});
...
test('the hook script exits 2 with a reason, as Claude Code expects', () => {
  ...
  assert.match(blocked.stderr, /Storyfront policy: Pushing/);
